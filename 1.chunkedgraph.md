...
> NOTE: The number of `workerDeployments` in `values.yaml` relates to the number of layers in the graph. For example, a graph with 6 layers must have 5 sections in `workerDeployments` (l2 - l6). To remove depoyments for a specific layer, uncomment the related section in `workerDeployments`. Also, if you add or remove worker deployments, `trackerDepoyments` need to be adapted accordingly. Note that the highest layer does not need a tracker because it has only one final root chunk. 

Some useful commands when working with the ingress
```shell
$ helm upgrade <release_name> . --debug // upgrades a deployment when changing values.yaml
```
