...
| `secret_files` | `{container_filename: local_path under ./secrets}`; must include `google-secret.json` — all Google clients authenticate with it, and so does the `pipeline` CLI itself, which drives the cluster as that account rather than as the operator's gcloud login. `cave-secret.json` is optional: meshing needs the file but never the token, so deploy mounts a placeholder when it is absent (provide a real one only for CAVE registration) |
| `secret_name` | name of the k8s Secret built from `secret_files` (default `cloud-volume-secrets`) |
| `images.pcg` / `images.l2cache` | container image per workload; `images.pcg` must be a public Docker Hub image following the **PCG image contract**, checked from the registry before deploy, resume, setup, mesh-meta, submit and sample (`pipeline image-preflight` runs it by hand); the tag itself is never parsed |
