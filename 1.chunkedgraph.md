...
worker_service_account = "cave-pipeline-worker-svc@neuromancer-seung-import.iam.gserviceaccount.com"
...
You should create service account credential via the cloud console for the service account to utilize helm. 
```
gcloud iam service-accounts keys create ${SERVICE_ACCOUNT_CREDENTIAL_NAME}.json --iam-account $PYCG_SERVICE_ACCOUNT_NAME@$PROJECT_NAME.iam.gserviceaccount.com
...
Update the values in the example_values.yaml file, including the environment variables, the service credentials from the terraform step, and fill out the configyamls section with the location of the files to ingest and the parameters appropriate for your dataset, as well as the image_repository and image_tag.  See the Pychunkedgraph repo for a list of [tags](https://github.com/CAVEconnectome/PyChunkedGraph/tags)
